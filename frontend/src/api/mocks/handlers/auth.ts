...
// base en memoria (se reinicia al refrescar)
let users: User[] = [
  { id: 1, email: 'admin@gym.com', nombre: 'Admin', password: 'admin123', roles: ['admin'] },
  { id: 2, email: 'user@gym.com',  nombre: 'Usuario', password: 'user123', roles: ['user'] },
];

let seq = 3;

// Helpers para JWT simple (fake)
const makeJwt = (payload: any) =>
  // payload base64 sin firmar (solo para demo)
...
export const authHandlers = [
  ...
  http.post('/auth/login', async ({ request }) => {
    ...
    const access_token = makeJwt({ sub: String(u.id), email: u.email, roles: u.roles, exp: Date.now() / 1000 + 3600 });
    const refresh_token = makeJwt({ sub: String(u.id), email: u.email, roles: u.roles, exp: Date.now() / 1000 + 3600 * 3 });
